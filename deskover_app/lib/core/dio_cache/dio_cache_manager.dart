...
import 'package:deskover_app/config/static_data.dart';
import 'package:deskover_app/core/dio_cache/core/dio_cache_ano.dart';
  ...
  _onRequest(RequestOptions options, RequestInterceptorHandler handler,) async {
    if (StaticData.isShowLog) {
      try {
        ...
        String logCurl = 'curl ${Uri.encodeFull(options.uri.toString())} --request ${options.method} $headerCurl--data ${options.data ?? '{}'}';
  ...
  _onError(DioError e, ErrorInterceptorHandler handler) async {
    if (StaticData.isShowLog) {
      try {
        ...
        String logCurl = 'curl ${Uri.encodeFull(e.requestOptions.uri.toString())} --request ${e.requestOptions.method} $headerCurl--data ${e.requestOptions.data ?? '{}'}';
